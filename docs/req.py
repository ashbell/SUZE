...
def fetchUrls( data ):
    ...
    for val in audios:
        ...
        audioUrl    = ''
        audioUrlGroup = []
        ## Audio_list 里面会包含有两个main_url,连在一起的，需要提取出来
        bitRate     = ''
        splited     = val.split(',')
        for item in splited:
            if 'vtype' in item:
            ...
            if 'main_url' in item:
                ...
                audioUrlGroup.append(audioUrl)
            if 'real_bitrate' in item:
                bitRate =  item.split(':')[1].strip('\"')
        
        for auVal in audioUrlGroup:
            audioAttrs = bitRate + '>' + audioType + '>' + auVal
...
""" 分析数据，取得最大分辨率的下载地址和音频地址, 传入参数
    { id:['360p>dash>url1', 720p>dash,url2] }
    返回参数:
    {id, videourl, audiourl }
"""
def getBestUrl( attrs = {} ):
    if len(attrs) == 0:
        print('Attrs: No valid data found!, exit')
        exit()
    ##print(attrs)
    ...
    for id in attrs.keys():
        ...
        print('---------------------------total of main_url----------------------')
        print('---------------------------%d----------------------' % len(strings))
        order = [ -1, -1, -1, -1, -1 ]
        for item in strings:
            strs = item.split('>')[0]+'>'+ item.split('>')[1] + '>'+ base64decode(item.split('>')[2])
            urlTmp.append(strs)
            if '360p' in item:
                order[4] = 0 
            if '480p' in item:
        ...
        if   order[0] == 4:
            ...
            print('Unkow resolution!')

        for flag in urlTmp:
            if bestVideoFlag in flag:
                bestVideoUrls.append(flag)
        bestVideoUrls.sort(reverse=True)
        bestV = bestVideoUrls[0]
        ...
        for flag in urlTmp:
            if 'mp4a' in flag:
                ...
                if 'p' in head:
                    ...
                    bestAudioUrls.append(flag)
        if len(bestAudioUrls) > 0:
            bestAudioUrls.sort(reverse=True)
            bestA = bestAudioUrls[0]


        if ('media-video-avc' in bestV) and \
           ('media-audio-und-mp4a' in bestA):
            print('This video has splited A/V')
            bestA = bestA.split('>')[2]
            bestV = bestV.split('>')[2]
            ##print(bestA)
            ##print('======================')
            ##print(bestV)
            return ['no_water', bestA, bestV]
        else:
            print('This video has no splited A/V')
            bestV = bestV.split('>')[2]
            return ['water', 'none',bestV ]
            ##print(bestV)
...
def downloadVideos( hasWater, audioUrl, videoUrl ):
    ...
    ##print(audioUrl)
    ##print(videoUrl)
    print('Call downloader  for video ID:\t' + videoID )
...
for url in urls:
    ...
    global videoID 
    videoID = getVideoId( url )
    videoAttrs = fetchUrls( data )
    videoPara = getBestUrl( videoAttrs )
    print(videoPara)
    downloadVideos(videoPara[0], videoPara[1], videoPara[2])
    logInfo1 = url + '\t----' + videoPara[0].strip('\n') +'\n\t'  \
               +videoPara[1].strip('\n') +'\n\t' + videoPara[2].strip('\n')+'\n'
    log.write(logInfo1)
    log.close()
